...
from layers import GraphAttentionreal2Shared8MoE
from layers import GraphAttentionreal2Shared8MoECoPRBias
...
class LoRALinear(nn.Module):
    def __init__(self, input_dim=1, high_dim=128, bottleneck_dim=16, output_dim=128):
        """
        input_dim: 输入维度（原始坐标维度）
        high_dim: 初始升维后的维度
        bottleneck_dim: 中间降维后的维度
        output_dim: 最终输出维度
        """
...
class Backbone_real2Shared8MoE_CoPRbias_QLinear64_LasttripleLayer(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_heads: int,
                 dropout: float) -> None:
        super(Backbone_real2Shared8MoE_CoPRbias_QLinear64_LasttripleLayer, self).__init__()
        ...
        self.a_emb_layer = LoRALinear(1,hidden_dim,64,hidden_dim)#TwoLayerMLP(input_dim=1, hidden_dim=hidden_dim, output_dim=hidden_dim)

        self.l2m_emb_layer = LoRALinear(3,hidden_dim,64,hidden_dim) #exp1 #TwoLayerMLP(input_dim=3, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.t2m_emb_layer = LoRALinear(4,hidden_dim,64,hidden_dim) #exp1#TwoLayerMLP(input_dim=4, hidden_dim=hidden_dim, output_dim=hidden_dim)


        self.m2m_h_emb_layer = LoRALinear(4,hidden_dim,64,hidden_dim)#TwoLayerMLP(input_dim=4, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.m2m_a_emb_layer = LoRALinear(3,hidden_dim,64,hidden_dim)#TwoLayerMLP(input_dim=3, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.m2m_s_emb_layer = LoRALinear(3,hidden_dim,64,hidden_dim)#TwoLayerMLP(input_dim=3, hidden_dim=hidden_dim, output_dim=hidden_dim)

        self.l2m_attn_layer = GraphAttentionreal2Shared8MoECoPRBias(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False, layer_idx=0)
        self.t2m_attn_layer = GraphAttentionreal2Shared8MoECoPRBias(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False, layer_idx=0)
        ...
        #                                                        if_self_attention=True, layer_idx=_) for _ in
        #                                         range(num_attn_layers)])
        ...
        self.m2m_h_attn_layers = nn.ModuleList([
            (GraphAttentionreal2Shared8MoE if _ == num_attn_layers - 1 else GraphAttention)(
                hidden_dim=hidden_dim, num_heads=num_heads,
                dropout=dropout, has_edge_attr=True,
                if_self_attention=True, layer_idx=_
        ...
        self.l2n_attn_layer = GraphAttentionreal2Shared8MoECoPRBias(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False, layer_idx=0)
        self.t2n_attn_layer = GraphAttentionreal2Shared8MoECoPRBias(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False, layer_idx=0)
    ...
    def forward(self, data: Batch, l_embs: torch.Tensor) -> torch.Tensor:
        ...
        m_embs_t, t2m_router_logits = self.t2m_attn_layer(x=[t_embs, m_embs], edge_index=t2m_edge_index,
                                                          edge_attr=t2m_edge_attr_embs)  # [(N1,...,Nb)*H*K,D]
        router_logits_list.append(t2m_router_logits)
        # l2m attention
        m_embs_l, l2m_router_logits = self.l2m_attn_layer(x=[l_embs, m_embs], edge_index=l2m_edge_index,
                                                          edge_attr=l2m_edge_attr_embs)  # [(N1,...,Nb)*H*K,D]
        router_logits_list.append(l2m_router_logits)
        ...
        for i in range(self.num_attn_layers):
            ...
            m_embs,none_or_logits = self.m2m_a_attn_layers[i](x=m_embs, edge_index=m2m_a_edge_index,
                                                                   edge_attr=m2m_a_edge_attr_embs)
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
            # m2m_h
            ...
            m_embs,none_or_logits = self.m2m_h_attn_layers[i](x=m_embs, edge_index=m2m_h_edge_index,
                                                                   edge_attr=m2m_h_edge_attr_embs)
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
            # m2m_s
            ...
            m_embs,none_or_logits = self.m2m_s_attn_layers[i](x=m_embs, edge_index=m2m_s_edge_index)
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
        ...
        # t2n attention
        t_embs = a_embs.reshape(-1, self.hidden_dim)  # [(N1,...,Nb)*H,D]
        n_embs_t, t2n_router_logits = self.t2n_attn_layer(x=[t_embs, n_embs], edge_index=t2n_edge_index,
                                                          edge_attr=t2n_edge_attr_embs)  # [(N1,...,Nb)*H*K,D]
        router_logits_list.append(t2n_router_logits)

        # l2m attention
        n_embs_l, l2n_router_logits = self.l2n_attn_layer(x=[l_embs, n_embs], edge_index=l2n_edge_index,
                                                          edge_attr=l2n_edge_attr_embs)  # [(N1,...,Nb)*H*K,D]
        router_logits_list.append(l2n_router_logits)
        ...
        for i in range(self.num_attn_layers):
            ...
            n_embs = n_embs.reshape(self.num_historical_steps, num_all_agent, self.num_modes,
                                    self.hidden_dim).transpose(1, 2).reshape(-1, self.hidden_dim)  # [H*K*(N1,...,Nb),D]
            n_embs,none_or_logits = self.n2n_a_attn_layers[i](x=n_embs, edge_index=n2n_a_edge_index,
                                                                   edge_attr=n2n_a_edge_attr_embs)
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
            # m2m_h
            n_embs = n_embs.reshape(self.num_historical_steps, self.num_modes, num_all_agent, self.hidden_dim).permute(
                1, 2, 0, 3).reshape(-1, self.hidden_dim)  # [K*(N1,...,Nb)*H,D]
            n_embs,none_or_logits = self.n2n_h_attn_layers[i](x=n_embs, edge_index=n2n_h_edge_index,
                                                                   edge_attr=n2n_h_edge_attr_embs)
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
            # m2m_s
            n_embs = n_embs.reshape(self.num_modes, num_all_agent, self.num_historical_steps,
                                    self.hidden_dim).transpose(0, 2).reshape(-1, self.hidden_dim)  # [H*(N1,...,Nb)*K,D]
            n_embs,none_or_logits = self.n2n_s_attn_layers[i](x=n_embs, edge_index=n2n_s_edge_index,
                                                                   edge_attr=n2n_s_edge_attr_embs)
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
        ...
        return traj_propose, traj_output, prob_output, router_logits_list  # [(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K]
