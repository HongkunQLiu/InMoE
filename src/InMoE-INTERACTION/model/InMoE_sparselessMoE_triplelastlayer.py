...
from modules import Backbone_lightMoE_QLinear64_LasttripleLayer as Backbone
...
def bad_switch_load_balancing_loss(router_logits: torch.Tensor, num_experts: int) -> torch.Tensor:
...
class InMoE_sparselessMoE_triplelastlayer(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 T_max: int,
                 **kwargs) -> None:
        super(InMoE_sparselessMoE_triplelastlayer, self).__init__()
    ...
    def training_step(self, data, batch_idx):
        traj_propose, traj_output,router_logits_list = self(data)  # [(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K,F],[(N1,...,Nb),H,K]
        ...
        for router_logits in router_logits_list:
            router_loss+=switch_load_balancing_loss(router_logits,4)
        alpha = 0.01
        loss = reg_loss_traj_propose + reg_loss_traj_refine + alpha * router_loss
    ...
    def validation_step(self, data, batch_idx):
        traj_propose, traj_output,_ = self(data)  # [(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K,F,2]
    ...
    def test_step(self, data, batch_idx):
        traj_propose, traj_output,_ = self(data)  # [(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K,F]
    ...
    def add_model_specific_args(parent_parser):
        parser = parent_parser.add_argument_group('InMoE_sparselessMoE_triplelastlayer')
        ...
        parser.add_argument('--T_max', type=int, default=82)
