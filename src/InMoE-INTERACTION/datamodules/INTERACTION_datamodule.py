...
class INTERACTIONDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        INTERACTIONDataset(self.root, 'train', self.train_transform, self.num_historical_steps, self.num_future_steps, self.sample_rate)
        INTERACTIONDataset(self.root, 'val', self.val_transform, self.num_historical_steps, self.num_future_steps,1)

    def setup(self, stage: Optional[str] = None) -> None:
        self.train_dataset = INTERACTIONDataset(self.root, 'train', self.train_transform, self.num_historical_steps, self.num_future_steps,self.sample_rate)
        self.val_dataset = INTERACTIONDataset(self.root, 'val', self.val_transform, self.num_historical_steps, self.num_future_steps,1)
