...
from layers import GraphAttentionSharedlessMoE
...
class Backbone_lightMoE_QLinear64_LasttripleLayer(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_heads: int,
                 dropout: float) -> None:
        super(Backbone_lightMoE_QLinear64_LasttripleLayer, self).__init__()
        ...
        self.a_emb_layer = LoRALinear(5,hidden_dim,64,hidden_dim)#TwoLayerMLP(input_dim=5, hidden_dim=hidden_dim, output_dim=hidden_dim)

        self.l2m_emb_layer = LoRALinear(3,hidden_dim,64,hidden_dim) #TwoLayerMLP(input_dim=3, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.t2m_emb_layer = LoRALinear(4,hidden_dim,64,hidden_dim) #TwoLayerMLP(input_dim=4, hidden_dim=hidden_dim, output_dim=hidden_dim)
        ...
        self.l2m_attn_layer = GraphAttentionSharedlessMoE(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False,layer_idx=0)
        self.t2m_attn_layer = GraphAttentionSharedlessMoE(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False,layer_idx=0)
        ...
        self.m2m_h_attn_layers = nn.ModuleList([
            (GraphAttentionSharedlessMoE if _ == num_attn_layers - 1 else GraphAttention)(
        ...
        self.l2n_attn_layer = GraphAttentionSharedlessMoE(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False, layer_idx=0)
        self.t2n_attn_layer = GraphAttentionSharedlessMoE(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False, layer_idx=0)
    ...
    def forward(self, data: Batch, l_embs: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.num_attn_layers):
            ...
            m_embs, none_or_logits = self.m2m_a_attn_layers[i](x=m_embs, edge_index=m2m_a_edge_index, edge_attr=m2m_a_edge_attr_embs)
            ...
            m_embs, none_or_logits = self.m2m_h_attn_layers[i](x=m_embs, edge_index=m2m_h_edge_index, edge_attr=m2m_h_edge_attr_embs)
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
            # m2m_s
        ...
        for i in range(self.num_attn_layers):
            ...
            n_embs = n_embs.reshape(self.num_historical_steps, num_all_agent, self.num_modes,
                                    self.hidden_dim).transpose(1, 2).reshape(-1, self.hidden_dim)  # [H*K*(N1,...,Nb),D]
            n_embs, none_or_logits = self.n2n_a_attn_layers[i](x=n_embs, edge_index=n2n_a_edge_index, edge_attr=n2n_a_edge_attr_embs)
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
            # m2m_h
            n_embs = n_embs.reshape(self.num_historical_steps, self.num_modes, num_all_agent, self.hidden_dim).permute(
                1, 2, 0, 3).reshape(-1, self.hidden_dim)  # [K*(N1,...,Nb)*H,D]
            n_embs, none_or_logits = self.n2n_h_attn_layers[i](x=n_embs, edge_index=n2n_h_edge_index, edge_attr=n2n_h_edge_attr_embs)
            # m2m_s
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
            n_embs = n_embs.reshape(self.num_modes, num_all_agent, self.num_historical_steps,
                                    self.hidden_dim).transpose(0, 2).reshape(-1, self.hidden_dim)  # [H*(N1,...,Nb)*K,D]
            n_embs,none_or_logits = self.n2n_s_attn_layers[i](x=n_embs, edge_index=n2n_s_edge_index, edge_attr=n2n_s_edge_attr_embs)
            if none_or_logits is not None:
                router_logits_list.append(none_or_logits)
        ...
        return traj_propose, traj_output,router_logits_list  # [(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K,F,2]