...
class GraphAttentionreal2Shared8MoECoPRBias(MessagePassing):
    def __init__(self,
                 hidden_dim: int,
                 num_heads: int,
                 dropout: float,
                 ...
                 layer_idx: int,
                 **kwargs) -> None:
        super(GraphAttentionreal2Shared8MoECoPRBias, self).__init__(aggr='add', node_dim=0, **kwargs)
        ...
        self.att_weigted_pr = nn.Sequential(
            nn.Linear(self.head_dim, self.head_dim),
            nn.LayerNorm(self.head_dim),
            nn.SiLU(),  # SiLU激活函数在Transformer中表现良好
        ...
        self.scale_layer = nn.Linear(self.head_dim, 1)

        nn.init.constant_(self.scale_layer.bias, 0.0)
        ...
        self.ShareMoE_config = MOEConfig(128, 8, 1,2)
    ...
    def message(self,
        ...
        qka_concat_feature = torch.cat([query_i, key_j, edge_attr], dim=-1)
        cogate_feature = self.cogate_feature_extractor(qka_concat_feature)
        coef = self.scale_layer(cogate_feature).squeeze(-1)


        weight = (query_i * key_j).sum(dim=-1) / scale +  coef * self.att_weigted_pr(edge_attr).squeeze(-1)
